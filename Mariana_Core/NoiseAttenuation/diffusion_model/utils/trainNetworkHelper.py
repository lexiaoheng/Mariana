...
class SimpleDiffusionTrainer(TrainerBase):
    ...
    def forward(self, model, *args, **kwargs):
        if self.mode == 'train' :
            ...
            noisy = kwargs["noisy"].astype(np.float32)
            t_seq = kwargs["t_seq"]

            if len(noisy.shape) == 3:
                num = noisy.shape[2]
                with tqdm(total=num, position=0, leave=True) as t:
                    for i in range(num):
                        features = torch.tensor(noisy[:, :, i]).unsqueeze(0).unsqueeze(0).to(self.device)
                        features = model(mode="generate", x_start=features, t=int(t_seq[i]),
                                whether_ddim=kwargs["ddim"], loss_type="huber")
                        noisy[:, :, i] = features.cpu().detach().squeeze(0).squeeze(0).numpy()

                        t.update(1)

                    return noisy
            else:
                with tqdm(total=1, position=0, leave=True) as t:
                    noisy = torch.tensor(noisy).unsqueeze(0).unsqueeze(0).to(self.device)
                    noisy = model(mode="generate", x_start=noisy, t=int(t_seq),
                            whether_ddim=kwargs["ddim"], loss_type="huber")
                    t.update(1)

                    return noisy.cpu().detach().squeeze(0).squeeze(0).numpy()
