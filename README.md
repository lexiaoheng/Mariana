...
- ### **Physics-Informed Acoustic Inversion**
  - #### Classification:
    ...
    - Seismic Acoustic Impedance Inversion (Post-Stacked Data, 2-D only)
    - Few-Shot Deep Learning Methods
    - Simultaneously perform wavelet extraction and acoustic impedance inversion.

  - #### Abstract:

    - We designed a deep learning framework to perform acoustic impedance inversion and seismic wavelet extraction simultaneously.
    - Considering the scarcity of well data, we proposed a physics-informed cross-learning strategy to impose effective constraints on the framework without forward or external initial model.    

  - #### Usage:

    ```python
    from Mariana_Core.Inversion import PIAI
    output, output_wavelet = PIAI(data, well_log, weighting='Equal', wavelet_len=301)
    ```
  
    - <b><span style="color: #2D3C81;"> data (Numpy.ndarray) </span></b> - Seismic data, 2D only. 
    - <b><span style="color: #2D3C81;"> well_log (Numpy.ndarray) </span></b> - Sparse well-logging matrix, has the same shape as the seismic data. If without well-logging data, the corresponding positions in the mssing_mask are all <b>0</b>.
    - <b><span style="color: #2D3C81;"> weighting (str, optional) </span></b> - The weight strategy for multiple losses. Default set to <b>'equal'</b>, meaning that all losses use equal weights; if set to <b>'UW'</b>, an uncertainty weighting is used.
    - <b><span style="color: #2D3C81;"> wavelet_len (int) </span></b> - The length of the extracted wavelet, which can be used to calculate its duration as <b> $duration = (length-1) \times dt$ </b>.
    - <b><span style="color: #2D3C81;"> output (Numpy.ndarray) </span></b> - Inverted acoustic impedance, has the same shape as the seismic data.
    - <b><span style="color: #2D3C81;"> output_wavelet (Numpy.ndarray) </span></b> - Extracted wavelet, 1D only.

  - #### Reference:

    - [Peng J, Wang X, Liu Y, et al. Physics-informed cross-learning for seismic acoustic impedance inversion and wavelet extraction. Computers and Geotechnics, 2026, 194: 108010.](https://www.sciencedirect.com/science/article/pii/S0266352X26001163)  
